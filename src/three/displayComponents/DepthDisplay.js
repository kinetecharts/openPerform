...
class DepthDisplay {
	...
	buildPC(particles) {
		...
		for ( var i = 0; i < this.positions.length; i += 3 ) {
			...
			this.idx++;
			// // positions
			var x = this.idx%this.dimensions.width;
			var y = this.y;
			var z = 0;
	...
	updateDepth(type, imgArray) {
		...
		if (this.mirror) {
			var newVerts = [];
			var i,j,temparray,chunk = this.dimensions.width;
			var imgArray = _.map(_.chunk(imgArray, this.dimensions.width), function(array) {
	...
	updateColor(type, imgArray) {
		...
		for ( var i = 0; i < colors.length; i += 3 ) {
			...
			if (imgArray[ idx ] > this.dimensions.near && imgArray[ idx ] < this.dimensions.far) {
				var c = this.mapRange(imgArray[ idx ], this.dimensions.far, this.dimensions.near, 0, 1);
