...
class VR {
	constructor(renderer, camera, parent, controls) {
		...
		var lazerGeo = new THREE.Geometry();
	...
	initEffect() {
		...
		if ( WEBVR.isAvailable() === true ) {
			var vrButton = WEBVR.getButton( this.vrEffect );
	...
	addController(id) {
		var controller = new THREE.ViveController( id );
		...
		controller.state = { axes:{x:0, y:0}, thumbpad:false, trigger:false, grips:false, menu:false };
	...
	compareTransforms(start, current) {
		var offset = current.midpoint.clone().sub(start.midpoint.clone());
		offset.x *= 2;
		offset.y *= 2;
		offset.z *= 2;
		return {
	...
	checkControllerState() {
		...
		if (_.filter(this.controllers, function(controller) { return controller.state.grips === true; }).length == 2) {
			...
			var currentTransform = this.calculateTransforms();
			...
			var transforms = this.compareTransforms(this.startingTransform, currentTransform);
		...
		if (_.filter(this.controllers, function(controller) { return controller.state.grip === true; }).length > 0) {
			this.trigger('gripClicked', {data:{controllers:this.controllers}});
		}

		if (_.filter(this.controllers, function(controller) { return controller.state.trigger === true; }).length > 0) {
			this.trigger('triggerClicked', {data:{controllers:this.controllers}});
		}

		if (_.filter(this.controllers, function(controller) { return controller.state.trigger === false; }).length > 0) {
			this.trigger('triggerReleased', {data:{controllers:this.controllers}});
		}

		var tool = this.tool;
		_.forEach(this.controllers, function(controller, id) {
	...
	intersectObjects( controller ) {
		...
		var lazerLine = controller.getObjectByName( 'line' );
		...
		if ( WEBVR.isAvailable() !== false ) {
			if (this.vrControls && this.vrEffect && this.vrEffect.isPresenting) {
				...
				for (var i = 0; i < this.controllers.length; i++) {
					...
					var pivot = this.controllers[i].getObjectByName( 'pivot' );
					if (pivot) {
	...
	find_angle(v1, v2, v3) {
		var dot = v1.x * v2.x + v1.y * v2.y + v3.z * v3.z;
