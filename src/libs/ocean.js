THREE.Ocean = function (renderer, camera, scene, options) {
  ...
  const fullscreeenVertexShader = THREE.ShaderLib.ocean_sim_vertex;

  // 1 - Horizontal wave vertices used for FFT
  const oceanHorizontalShader = THREE.ShaderLib.ocean_subtransform;
  const oceanHorizontalUniforms = THREE.UniformsUtils.clone(oceanHorizontalShader.uniforms);
  ...
  const oceanVerticalShader = THREE.ShaderLib.ocean_subtransform;
  const oceanVerticalUniforms = THREE.UniformsUtils.clone(oceanVerticalShader.uniforms);
  ...
  const initialSpectrumShader = THREE.ShaderLib.ocean_initial_spectrum;
  const initialSpectrumUniforms = THREE.UniformsUtils.clone(initialSpectrumShader.uniforms);
  ...
  const phaseShader = THREE.ShaderLib.ocean_phase;
  const phaseUniforms = THREE.UniformsUtils.clone(phaseShader.uniforms);
  ...
  const spectrumShader = THREE.ShaderLib.ocean_spectrum;
  const spectrumUniforms = THREE.UniformsUtils.clone(spectrumShader.uniforms);
  ...
  const normalShader = THREE.ShaderLib.ocean_normals;
  const normalUniforms = THREE.UniformsUtils.clone(normalShader.uniforms);
  ...
  const oceanShader = THREE.ShaderLib.ocean_main;
  const oceanUniforms = THREE.UniformsUtils.clone(oceanShader.uniforms);
...
THREE.Ocean.prototype.generateSeedPhaseTexture = function () {
  ...
  this.pingPhase = true;
  const phaseArray = new window.Float32Array(this.resolution * this.resolution * 4);
...
THREE.Ocean.prototype.renderSpectrumFFT = function () {
  ...
  const iterations = Math.log(this.resolution) / Math.log(2); // log2
