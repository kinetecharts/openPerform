...
THREE.GPUParticleSystem = function (options) {
  ...
  this.init = function () {
    for (let i = 0; i < 100; i++) {
      const c = new THREE.GPUParticleContainer(self.PARTICLES_PER_CONTAINER, self);
  ...
  this.spawnParticle = function (options) {
    ...
    const currentContainer = self.particleContainers[Math.floor(self.PARTICLE_CURSOR / self.PARTICLES_PER_CONTAINER)];
...
THREE.GPUParticleContainer = function (maxParticles, particleSystem) {
  ...
  const particlesPerArray = Math.floor(self.PARTICLE_COUNT / self.MAX_ATTRIBUTES);
  ...
  this.spawnParticle = function (options) {
    ...
    position = options.position !== undefined ? position.copy(options.position) : position.set(0.0, 0.0, 0.0);
    velocity = options.velocity !== undefined ? velocity.copy(options.velocity) : velocity.set(0.0, 0.0, 0.0);
