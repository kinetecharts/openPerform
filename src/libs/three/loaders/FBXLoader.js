...
(function () {
  ...
  THREE.FBXLoader.prototype.parse = function (text) {
    ...
    const nodes = new FBXParser().parse(text);
  ...
  THREE.FBXLoader.prototype.parseGeometry = function (node, nodes) {
    ...
    let texture;
    const texs = this.textures.getById(nodes.searchConnectionParent(geo.id));
  ...
  THREE.FBXLoader.prototype.addAnimation = function (mesh, matrices, animations) {
    const animationdata = {
      name: 'animationtest', fps: 30, length: animations.length, hierarchy: [],
    };

    for (var i = 0; i < mesh.geometry.bones.length; ++i) {
      let name = mesh.geometry.bones[i].name;
      name = name.replace(/.*:/, '');
      animationdata.hierarchy.push({ parent: mesh.geometry.bones[i].parent, name, keys: [] });
    }

    const hasCurve = function (animNode, attr) {
    ...
    const hasKeyOnFrame = function (attrNode, frame) {
      const x = isKeyExistOnFrame(attrNode.curves.x, frame);
      const y = isKeyExistOnFrame(attrNode.curves.y, frame);
      const z = isKeyExistOnFrame(attrNode.curves.z, frame);

      return x && y && z;
    };

    var isKeyExistOnFrame = function (curve, frame) {
      const value = curve.values[frame];
      return value !== undefined;
    };


    const genKey = function (animNode, bone) {
      ...
      try {
        ...
        if (hasCurve(animNode, 'R') && hasKeyOnFrame(animNode.R, frame)) {
          const rx = degToRad(animNode.R.curves.x.values[frame]);
          const ry = degToRad(animNode.R.curves.y.values[frame]);
          const rz = degToRad(animNode.R.curves.z.values[frame]);
          const eul = new THREE.Vector3(rx, ry, rz);
          const rot = quatFromVec(eul.x, eul.y, eul.z);
    ...
    for (frame = 0; frame < animations.frames; frame++) {
      for (i = 0; i < bones.length; i++) {
        const bone = bones[i];
        const animNode = animations.curves[i];
  ...
  THREE.FBXLoader.prototype.loadFile = function (url, onLoad, onProgress, onError, responseType) {
    ...
    const request = loader.load(url, (result) => {
      onLoad(result);
    }, onProgress, onError);
  ...
  FBXParser.prototype = {
    ...
    parse(text) {
      ...
      for (const line in split) {
        ...
        if (match) {
          ...
          const nodeAttrs = match[2].split(',').map(element => element.trim().replace(/^"/, '').replace(/"$/, ''));
    ...
    parseNodeSpecialProperty(line, propName, propValue) {
      ...
      const props = propValue.split('",').map(element => element.trim().replace(/^\"/, '').replace(/\s/, '_'));
  ...
  Weights.prototype.parseCluster = function (node, id, entry) {
    const _p = node.searchConnectionParent(id);
    const _indices = toInt(entry.subNodes.Indexes.properties.a.split(','));
    const _weights = toFloat(entry.subNodes.Weights.properties.a.split(','));
    const _transform = toMat44(toFloat(entry.subNodes.Transform.properties.a.split(',')));
    const _link = toMat44(toFloat(entry.subNodes.TransformLink.properties.a.split(',')));
  ...
  Weights.prototype.parse = function (node, bones) {
    ...
    const deformers = node.Objects.subNodes.Deformer;

    const clusters = {};
    for (const id in deformers) {
      if (deformers[id].attrType === 'Cluster') {
        ...
        const cluster = this.parseCluster(node, id, deformers[id]);
        const boneId = node.searchConnectionChildren(cluster.id)[0];
    ...
    const weights = [];
    const hi = bones.hierarchy;
    for (let b = 0; b < hi.length; ++b) {
      const bid = hi[b].internalId;
      ...
      for (let v = 0; v < clst.indices.length; ++v) {
        ...
        // indices
        const affect = node.searchConnectionChildren(clst.id);
    ...
    for (let i = 0; i < weights.length; i++) {
      const indicies = new THREE.Vector4(
  ...
  Bones.prototype.parseHierarchy = function (node) {
    ...
    const models = objects.subNodes.Model;
    ...
    for (let i = 0; i < bones.length; ++i) {
      ...
      const p = node.searchConnectionParent(bone.id)[0];
      let t = [0.0, 0.0, 0.0];
      ...
      let name = bone.attrName;
      name = name.replace(/:/, '');
      name = name.replace(/_/, '');
      name = name.replace(/-/, '');
      this.hierarchy.push({
        parent: p, name, pos: t, rotq: r, scl: s, internalId: bone.id,
      });
    }
  ...
  Bones.prototype.reindexParentId = function () {
    for (let h = 0; h < this.hierarchy.length; h++) {
      for (let ii = 0; ii < this.hierarchy.length; ++ii) {
  ...
  Bones.prototype.restoreBindPose = function (node) {
    const bindPoseNode = node.Objects.subNodes.Pose;
    if (bindPoseNode === undefined) {
      return;
    }

    const poseNode = bindPoseNode.subNodes.PoseNode;
    const localMatrices = {}; // store local matrices, modified later( initialy world space )
    const worldMatrices = {}; // store world matrices

    for (let i = 0; i < poseNode.length; ++i) {
      const rawMatLcl = toMat44(poseNode[i].subNodes.Matrix.properties.a.split(','));
      const rawMatWrd = toMat44(poseNode[i].subNodes.Matrix.properties.a.split(','));
    ...
    for (let h = 0; h < this.hierarchy.length; ++h) {
      const bone = this.hierarchy[h];
      const inId = bone.internalId;
      ...
      var parentId;
      const parentNodes = node.searchConnectionParent(inId);
      for (let pn = 0; pn < parentNodes.length; ++pn) {
  ...
  Geometry.prototype.parse = function (geoNode) {
    ...
    if (this.getPolygonTopologyMax() > 3) {
      this.indices = this.convertPolyIndicesToTri(this.indices, this.getPolygonTopologyArray());
    }
  ...
  Geometry.prototype.getVertices = function () {
    ...
    const rawTextVert	= this.node.subNodes.Vertices.properties.a;
    const vertices = rawTextVert.split(',').map(element => parseFloat(element));
  ...
  Geometry.prototype.getPolygonVertexIndices = function () {
    ...
    const rawTextIndices = this.node.subNodes.PolygonVertexIndex.properties.a;
    const indices = rawTextIndices.split(',');
  ...
  Geometry.prototype.convertPolyIndicesToTri = function (indices, strides) {
    ...
    while (i < indices.length) {
      ...
      // CAUTIN: NG over 6gon
      for (let j = 0; j <= (currentStride - 3); j++) {
        res.push(indices[i]);
  ...
  UV.prototype._parseText = function (node) {
    ...
    const uvIndex = uvNode.subNodes.UVIndex.properties.a;
    const uvs = uvNode.subNodes.UV.properties.a;
    const uvMap = uvNode.properties.MappingInformationType;
    const uvRef = uvNode.properties.ReferenceInformationType;
  ...
  UV.prototype.parse = function (node, geo) {
    ...
    const mappingType = this.getMap(node);
    const refType = this.getRef(node);
    const indices = this.getIndex(node);

    const strides = geo.getPolygonTopologyArray();
  ...
  Normal.prototype._parseText = function (node) {
    const normalNode = this.getNode(node);
    ...
    const refType = normalNode.properties.ReferenceInformationType;

    const rawTextNormals = normalNode.subNodes.Normals.properties.a;
  ...
  Normal.prototype.parse = function (topnode, geo) {
    let normals = this.getNormal(topnode);
    const normalNode = this.getNode(topnode);
    const mappingType = this.getMap(topnode);
    const refType = this.getRef(topnode);

    const indices = geo.getPolygonVertexIndices(topnode);
    const strides = geo.getPolygonTopologyArray(topnode);
  ...
  AnimationCurve.prototype.fromNode = function (curveNode) {
    ...
    this.times = this.times.map(element => FBXTimeToSeconds(element));
  ...
  AnimationNode.prototype.fromNode = function (allNodes, node, bones) {
    ...
    if (this.attr.match(/S|R|T/)) {
      for (const attrKey in node.properties) {
        if (attrKey.match(/X/)) {
          this.attrX = true;
        }
        if (attrKey.match(/Y/)) {
    ...
    for (let i = this.containerIndices.length - 1; i >= 0; --i) {
      const boneId = bones.searchRealId(this.containerIndices[i]);
      if (boneId >= 0) {
  ...
  Animation.prototype.parse = function (node, bones) {
    const rawNodes = node.Objects.subNodes.AnimationCurveNode;
    const rawCurves = node.Objects.subNodes.AnimationCurve;

    // first: expand AnimationCurveNode into curve nodes
    const curveNodes = [];
    for (var key in rawNodes) {
      if (key.match(/\d+/)) {
        const a = (new AnimationNode()).fromNode(node, rawNodes[key], bones);
        curveNodes.push(a);
      }
    }
    ...
    for (key in rawCurves) {
      if (key.match(/\d+/)) {
        const c = (new AnimationCurve()).fromNode(rawCurves[key]);
        ac.push(c);
        max = c.getLength() ? c.getLength() : max;

        const parentId = node.searchConnectionParent(c.id)[0];
        let axis = node.searchConnectionType(c.id, parentId);
    ...
    // forth:
    for (const t in tmp) {
      const id = tmp[t].containerBoneId;
  ...
  Textures.prototype.add = function (tex) {
    ...
    for (let i = 0; i < tex.parentIds.length; ++i) {
  ...
  Textures.prototype.parse = function (node, bones) {
    const rawNodes = node.Objects.subNodes.Texture;

    for (const n in rawNodes) {
      const tex = (new Texture()).parse(rawNodes[n], node);
      this.add(tex);
    }
  ...
  Texture.prototype.searchParents = function (id, nodes) {
    const p = nodes.searchConnectionParent(id);
  ...
  var quatFromVec = function (x, y, z) {
    const euler = new THREE.Euler(x, y, z, 'ZYX');
  ...
  var toRad = function (arr) {
    return arr.map(element => degToRad(element));
  };
