...
THREE.SceneLoader.prototype = {
  ...
  load(url, onLoad, onProgress, onError) {
    ...
    scope.parse(JSONScene, onLoad, url);

    // }, onProgress, onError );
  },

  addGeometryHandler(typeID, loaderClass) {
    this.geometryHandlers[typeID] = { loaderClass };
  },

  addHierarchyHandler(typeID, loaderClass) {
    this.hierarchyHandlers[typeID] = { loaderClass };
  },
  ...
  parse(json, callbackFinished, url) {
    ...
    function handle_children(parent, children) {
      ...
      for (const objID in children) {
        // check by id if child has already been handled,
        // if not, create new object

        let object = result.objects[objID];
        const objJSON = children[objID];
        ...
        if (object === undefined) {
          ...
          if (objJSON.type && (objJSON.type in scope.hierarchyHandlers)) {
            if (objJSON.loading === undefined) {
              ...
              const loader = scope.hierarchyHandlers[objJSON.type].loaderObject;
            ...
            const color = objJSON.color;
            const intensity = objJSON.intensity;
            const distance = objJSON.distance;
            const position = objJSON.position;
            const rotation = objJSON.rotation;
          ...
          if (object) {
            if (objJSON.userData !== undefined) {
              for (const key in objJSON.userData) {
                const value = objJSON.userData[key];
                object.userData[key] = value;
              }
            }

            if (objJSON.groups !== undefined) {
              for (let i = 0; i < objJSON.groups.length; i++) {
                const groupID = objJSON.groups[i];
    ...
    const generateTextureCallback = function (count) {
      return function () {
    ...
    function traverse_json_hierarchy(objJSON, callback) {
      ...
      if (objJSON.children !== undefined) {
        for (const objChildID in objJSON.children) {
    ...
    for (const objID in data.objects) {
      traverse_json_hierarchy(data.objects[objID], (objJSON) => {
        if (objJSON.type && (objJSON.type in scope.hierarchyHandlers)) {
          counter_models += 1;
    ...
    for (geoID in data.geometries) {
      ...
      if (geoJSON.type === 'cube') {
        ...
        if (modelJson) {
          const jsonLoader = this.geometryHandlers.ascii.loaderObject;
          const model = jsonLoader.parse(modelJson, texture_path);
    ...
    for (textureID in data.textures) {
      ...
      if (Array.isArray(textureJSON.url)) {
        ...
        for (let n = 0; n < textureJSON.url.length; n++) {
          scope.onLoadStart();
        }
      } else {
        counter_textures += 1;
    ...
    for (textureID in data.textures) {
      ...
      if (Array.isArray(textureJSON.url)) {
        const count = textureJSON.url.length;
        const urls = [];
        ...
        if (loader !== null) {
          ...
          if (textureJSON.mapping !== undefined) { texture.mapping = textureJSON.mapping; }
        } else {
    ...
    for (matID in data.materials) {
      ...
      for (parID in matJSON.parameters) {
        if (parID === 'envMap' || parID === 'map' || parID === 'lightMap' || parID === 'bumpMap' || parID === 'normalMap' || parID === 'alphaMap') {
          ...
          const v3 = matJSON.parameters[parID];
          matJSON.parameters[parID] = new THREE.Vector3(v3[0], v3[1], v3[2]);
        } else if (parID === 'normalScale') {
          const v2 = matJSON.parameters[parID];
    ...
    for (matID in data.materials) {
      ...
      if (matJSON.parameters.materials) {
        ...
        for (var i = 0; i < matJSON.parameters.materials.length; i++) {
          const label = matJSON.parameters.materials[i];
