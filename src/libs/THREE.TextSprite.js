const TextTexture = require('./THREE.TextTexture.min.js');

(function (THREE) {
  const onBeforeRender = function (renderer, a, camera) {
  ...
  THREE.TextSprite = class extends THREE.Sprite {
    ...
    computeOptimalFontSize(renderer, camera) {
      if (renderer.domElement.width && renderer.domElement.height && this.material.map.linesCount) {
        const distance = this.getWorldPosition().distanceTo(camera.getWorldPosition());
        if (distance) {
          const heightInPixels = this.getWorldScale().y * renderer.domElement.height / distance;
          if (heightInPixels) {
